...
function FileUpload( $resourceType, $currentFolder, $sCommand )
{
    ...
    if ( isset( $_FILES['NewFile'] ) && !is_null( $_FILES['NewFile']['tmp_name'] ) )
    {
        ...
        // 图片内容安全检查
        $ims_options = isset($GLOBALS['_CFG']['ims_plugin']) ? json_decode($GLOBALS['_CFG']['ims_plugin'], true) : array();
        if (!empty($ims_options) && $ims_options['switch'] === '1') {
            $ims_images = array_unique(array($oFile['tmp_name']));
            include_once ROOT_PATH . 'includes/cls_tencentcloud_ims.php';
            $tencent_ims = new tencent_ims($ims_options);
            if (!empty($ims_images) && !$tencent_ims->AuditImages("", $ims_images)) {
                SendUploadResults('1', $oFile['tmp_name'], $oFile['name'], '操作失败，上传的图片中存在违规内容！');
                exit;
            }
        }
        // Map the virtual path to the local server path.

 ...
 * @return bool
 */
function is_tms_enable()
{
    $tms_options = get_tms_options();
    return !empty($tms_options) && $tms_options['switch'] === '1';
}
...
function get_tms_options()
{
    return isset($GLOBALS['_CFG']['tms_plugin']) ? json_decode($GLOBALS['_CFG']['tms_plugin'], true) : array();
}
 ...
 *
 * @param $cmt
 * @return bool|Exception|\TencentCloud\Cms\V20190321\Models\TextModerationResponse|\TencentCloud\Common\Exception\TencentCloudSDKException
 * @throws Exception
 */
function textModeration($content)
{
    if (function_exists('is_tms_enable') && is_tms_enable() && $content !== '') {
        $tms_options = get_tms_options();
        include_once ROOT_PATH . 'includes/cls_tencentcloud_tms.php';
        $tencent_tms = new tencent_tms($tms_options);
        return $tencent_tms->textModeration($content);
    }
    return true;
}
